# 2.GUIを使って、保存ファイル名を指定でき、録画ボタンを押すと映像を取り込んで、次に停止ボタンを押すまでの間の動画を指定したファイル名で保存するプログラムを作成する。ボタンは、一つにして録画と停止をトグルで表示してもよいし、録画ボタンと停止ボタンを二つ用意してもよい。ただし、録画と停止が同時に有効になる状態がないようにボタンを制御すること。
